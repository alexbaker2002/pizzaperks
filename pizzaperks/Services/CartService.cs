	...
	public class CartService : ICartService
	{
		...
		public async Task ClearCartAsync(Cart cart)
		{
			...
			await Task.Run(async () =>
			{
				foreach (CartProduct product in cart.Products)
				{
					foreach (OrderedIngredient ingredient in product.Ingredients)
					{
						await RemoveIngredientFromCartProductAsync(ingredient);
					}
					await DeleteFromCartProductAsync(product);
